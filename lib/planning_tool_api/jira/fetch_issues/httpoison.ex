...
defmodule PlanningToolApi.JIRA.FetchIssues.HTTPoison do
  @behaviour PlanningToolApi.JIRA.FetchIssues
  ...
  defp pool_for_get_issue() do
    poolboy_config = [
      ...
      {:worker_module, PlanningToolApi.JIRA.FetchIssues.HTTPoison.FetchIssueWorker},
      {:size, 0},
      {:max_overflow, 10}
    ]
  ...
  defp pooled_get_issue(key) do
    :poolboy.transaction(:get_issue_workers_pool, fn(pid) ->
      PlanningToolApi.JIRA.FetchIssues.HTTPoison.FetchIssueWorker.execute(pid, key)
    end)
  end
  ...
  defmodule FetchIssueWorker do
    ...
    def get_issue_module() do
      Application.get_env(:planning_tool_api, :jira_get_issue)
    end
  end
end
